...
$(document).ready(function () {
  ...
  (function () {
    var _document = $(document),
        ...
        _body = $('body'),
        header = $('.js-header'),
        container = header.find('.js-header-container'),
        burger = header.find('.js-header-burger'),
        link = header.find('.js-header-link'),
        overlay = header.find('.js-header-overlay'),
        section = $('.js-section');

    burger.on('click', function (e) {
      e.stopPropagation();
      _body.toggleClass('no-scroll');
    ...
    _window.scroll(function () {
      ...
      section.each(function () {
        var _this = $(this),
            ...
            anchor = _this.find('.js-section-anchor'),
            anchorTop = anchor.offset().top - _window.height() * .75,
            id = anchor.attr('id');

        var activeLink = header.find('.js-header-link[href="#' + id + '"]');
        if (scrollPos >= anchorTop && scrollPos <= anchorTop + _thisHeight) {
          activeLink.attr('style', _this.attr('style'));
  ...
  (function () {
    ...
    if (sidebar.length) {
      var sidebarBtn = sidebar.find('.js-sidebar-btn'),
          licenseInput = sidebar.find('.js-licenses-input'),
          licenceOptions = sidebar.find('.js-licenses-options'),
          head = sidebar.find('.js-sidebar-head'),
          wrap = sidebar.find('.js-sidebar-wrap'),
          buy = sidebar.find('.js-sidebar-buy'),
          code = licenseInput.first().data('btn-code');

      licenseInput.on('change', function () {
        var _this = $(this),
            _thisOptions = _this.next().find('.js-licenses-options');
        sidebarBtn.text($(this).data('btn-text'));
        code = $(this).data('btn-code');
        licenceOptions.slideUp();
        _thisOptions.slideDown();
      });

      Paddle.Setup({ vendor: 36856 });
      ...
      _window.scroll(function () {
        var scrollPos = _window.scrollTop(),
            ...
            buyHeight = buy.outerHeight(),
            headerHeight = $('.js-header').height(),
            layout = $('.js-layout'),
            layoutHeight = layout.outerHeight(),
            layoutPaddingBottom = parseInt(layout.css('paddingBottom'));

        if (scrollPos >= headHeight) {
          wrap.css('height', buyHeight);
          buy.addClass('fixed');
        } else {
        ...
        if (scrollPos >= sidebarHeight - _window.height() + headerHeight) {
          sidebar.addClass('fixed');
        } else {
          sidebar.removeClass('fixed');
        }

        if (scrollPos >= headerHeight + layoutHeight - layoutPaddingBottom - _window.height()) {
          sidebar.addClass('absolute');
        } else {
          sidebar.removeClass('absolute');
        }

        if (scrollPos >= layoutHeight - layoutPaddingBottom - buyHeight) {
          buy.addClass('absolute');
        } else {
          buy.removeClass('absolute');
        }
      });
    }
  })();
  ...
  (function () {
    var gallery = $('.js-gallery'),
        link = gallery.find('.js-gallery-link'),
  ...
  // youtube videos
  function findVideos() {
    var videos = document.querySelectorAll('.js-video-container');
